...
import CompanyObjectiveListItem from '../components/TheCompanyObjectiveListItem.vue'
...
export default {
  data() {
    return {
      ...
      // tooltips
      headerTooltip: 'Your company objectives should align with your vision and mission in order for employees to best guide their own actions and decisions. A company might use objectives to inform yearly strategies that each department will execute. An objective, on the other hand, defines the specific, measurable actions each team employee must take to achieve the overall goal.',
      ...
      <div class="md:flex md:items-center md:justify-between md:space-x-5">
        <div class="flex items-start space-x-5">
          ...
          <div class="pt-1.5 relative">
            <h1 class="text-2xl font-semibold text-gray-900 dark:text-gray-200">Company Objectives</h1>
          <p class="text-sm font-medium text-gray-500 dark:text-gray-400/80 mt-0.5">
        ...
        <div class="justify-stretch mt-6 flex flex-col-reverse space-y-4 space-y-reverse sm:flex-row-reverse sm:justify-end sm:space-y-0 sm:space-x-3 sm:space-x-reverse md:mt-0 md:flex-row md:space-x-3">
          <PrimaryButton>New Objective</Primarybutton>
        </div>
      </div>
      <!-- Goals section -->
      <div class="mt-2 sm:mt-8">
        <div class="xl:flex md:items-end md:justify-between">
          <div v-if="showClosedGoals == false">
            <h2 class="text-lg font-semibold text-gray-900 dark:text-gray-200">Current Objectives</h2>
            <div class="flex flex-col sm:flex-row mt-2 sm:mt-0.5 space-y-2 sm:space-y-0 sm:space-x-6">
              <div class="flex items-center">
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400/80">          
                  {{objectives.length}} current Objectives
                </p>
              </div>
              <div class="flex items-center">
                <svg class="mr-1.5 h-4 w-4 flex-shrink-0 text-gray-400 dark:text-gray-400/80" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
          ...
          <div v-else>
            <h2 class="text-lg font-semibold text-gray-900 dark:text-gray-200">Current and archived objectives</h2>
              ...
              <div class="mt-2 flex space-x-3 md:mt-2 xl:mt-0">
                ...
                <SwitchGroup as="div" class="flex items-center w-full justify-between">
                  <SwitchLabel as="span" class="mr-3">
                    <span class="text-sm font-medium text-gray-500 dark:text-gray-400/80">Archivierte Zielvorgaben anzeigen</span>
                  </SwitchLabel>
                  <Switch v-model="showClosedGoals" :class="[showClosedGoals ? 'bg-sky-400' : 'bg-gray-200 dark:bg-gray-700', 'relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-sky-400 focus:ring-offset-2']">
                    <span aria-hidden="true" :class="[showClosedGoals ? 'translate-x-5' : 'translate-x-0', 'pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white dark:bg-gray-900 shadow ring-0 transition duration-200 ease-in-out']" />
                  </Switch>
                </SwitchGroup>
              </div>
            </div>
        ...
        <div class="mt-6">
          ...
          <ul v-else role="list" class="space-y-4">
            <CompanyObjectiveListItem v-for="objective in objectives" :key="objective.id"
             :objective="objective">
            </CompanyObjectiveListItem>
          </ul>
        </div>
