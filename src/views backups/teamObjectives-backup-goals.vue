...
import ObjectiveListItem from '../components/TheObjectiveListItem.vue'
import ObjectiveColorButtonVue from '../components/TheObjectiveColorButton.vue'
import GoalsData from '/src/goals.json'
export default {
  data() {
    return {
      // goals data
      ...
      // tooltips
      headerTooltip: 'Your goals should align with your vision and mission in order for employees to best guide their own actions and decisions. A company might use goals to inform yearly strategies that each department will execute. An objective, on the other hand, defines the specific, measurable actions each team employee must take to achieve the overall goal.',
...
<template>
  <div>
    ...
    <div class="max-w-5xl mx-auto">
      ...
      <div class="md:flex md:items-center md:justify-between md:space-x-5">
        <div class="flex items-start space-x-5">
          ...
          <div class="pt-1.5 relative">
            <h1 class="text-2xl font-semibold text-gray-900">Company Objective</h1>
          <p class="text-sm font-medium text-gray-500 mt-0.5">
            A company objective is an achievable outcome that is typically broad and long-term.
      ...
      <!-- Goals section -->
      <div class="mt-8">
        <div class="md:flex md:items-end md:justify-between md:space-x-5">
          <div v-if="showClosedGoals == false">
            <h2 class="text-lg font-semibold text-gray-900">Laufende Ziele</h2>
            <p class="mt-0.5 text-sm text-gray-500">          
              Übersicht aller laufenden Ziele der Organisation.
            </p>
          </div>
          <div v-else>
            <h2 class="text-lg font-semibold text-gray-900">Laufende und abgeschlossene Ziele</h2>
            <p class="mt-0.5 text-sm text-gray-500">          
              Übersicht aller laufenden und abgeschlossenen Ziele der Organisation.
            </p>
          </div>
              ...
              <div class="mt-2 flex space-x-3 md:mt-0">
                ...
                <SwitchGroup as="div" class="flex items-center w-full justify-between">
                  <SwitchLabel as="span" class="mr-3">
                    <span class="text-sm font-medium text-gray-500">Abgeschlossene Ziele anzeigen</span>
                  </SwitchLabel>
        ...
        <div class="mt-6 space-y-8">
          <div v-for="goal in goals" :key="goal.id" class="overflow-hidden bg-white rounded-lg">
            <!-- Wrapper -->
                ...
                <div class="">
                  <div class="group flex items-start space-x-3">
                    <div class="flex-shrink-0">
                      <span :class="['inline-flex items-center justify-center h-14 w-14 rounded-lg', 'bg-'+goal.color]">
                    ...
                    <div class="flex-shrink-0">
                      <GoalStatusBadge :goalStatus="goal.status"></GoalStatusBadge>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div class="px-4 py-5 sm:px-6 flex-grow">
                <dl class="grid grid-cols-1 gap-x-4 gap-y-8">
                  <div class="col-span-1">
                    <!-- Empty state -->
                    <div v-if="goal.objectives.length == 0" class="text-center">
                      ...
                      <div class="mt-4">
                        <button @click="openStrengthsModal()" type="button" :class="['inline-flex items-center rounded-md border border-transparent', 'bg-'+goal.color+' px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-'+goal.color+' focus:outline-none focus:ring-2 focus:ring-'+goal.color+' focus:ring-offset-2']">
                    ...
                    <ul v-else class="">
                      <div class="w-full">
                        <h2 class="text-md font-medium text-gray-900">Objectives</h2>
                        <div class="flex items-center">
                          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="text-gray-400/80 mt-1 mr-1 w-4 h-4">
                            <path fill-rule="evenodd" d="M1 2.75A.75.75 0 011.75 2h16.5a.75.75 0 010 1.5H18v8.75A2.75 2.75 0 0115.25 15h-1.072l.798 3.06a.75.75 0 01-1.452.38L13.41 18H6.59l-.114.44a.75.75 0 01-1.452-.38L5.823 15H4.75A2.75 2.75 0 012 12.25V3.5h-.25A.75.75 0 011 2.75zM7.373 15l-.391 1.5h6.037l-.392-1.5H7.373zm7.49-8.931a.75.75 0 01-.175 1.046 19.326 19.326 0 00-3.398 3.098.75.75 0 01-1.097.04L8.5 8.561l-2.22 2.22A.75.75 0 115.22 9.72l2.75-2.75a.75.75 0 011.06 0l1.664 1.663a20.786 20.786 0 013.122-2.74.75.75 0 011.046.176z" clip-rule="evenodd" />
                          </svg>
                          <span class="mt-1 text-sm text-gray-500 mr-1">{{goal.objectives.length}}</span>
                          <p class="mt-1 text-sm text-gray-500">Objectives assigned to this business goal.</p>
                        </div>
                        
                        <ul role="list" class="mt-4 space-y-2 -mx-3.5">
                          <!-- initial design
                          ...
                          -->
                          <ObjectiveListItem v-for="objective in goal.objectives" :key="objective.id"
                            :id="objective.id"
                            :name="objective.name"
                        ...
                        <div class="mt-6 flex">
                          <a href="#" :class="['text-sm font-medium text-'+goal.color+' hover:text-'+goal.color]">
                ...
                <div class="flex w-full items-center">
                  <div class="flex flex-1 flex-wrap">
                    <p class="flex text-sm text-gray-400 truncate">
                      ...
                      <span v-if="goal.lastUpdated == null">
                        Never updated
                      </span>
                      <span v-else>
                      Last updated on
                      {{goal.lastUpdated.toLocaleString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}}
                      </span>
